...
function _runAnalysis(){
  ...
  const firstR  = recs.filter(r=>r.turn==='first');
  const secondR = recs.filter(r=>r.turn==='second');
  const fW = firstR.filter(r=>r.result==='win').length;
  const sW = secondR.filter(r=>r.result==='win').length;
  ...
  recs.forEach(r=>{
    ...
    win?d.w++:d.l++;
    if(r.turn==='first'){ win?d.fw++:d.fl++; }
    else if(r.turn==='second'){ win?d.sw++:d.sl++; }
  });
  ...
  if(deckEntries.length){
    ...
    deckEntries.forEach(([id,d])=>{
      ...
      h+=`<tr>
        <td style="text-align:left"><span style="display:inline-flex;align-items:center;gap:4px">${clsIcon(dk?.className,13)}${dk?.name||'?'}</span></td>
  ...
  if(Object.keys(byCombo).length){
    ...
    Object.entries(byCombo).sort((a,b)=>(b[1].w+b[1].l)-(a[1].w+a[1].l)).forEach(([k,d])=>{
      ...
      h+=`<tr>
        ...
        <td>${t2}</td><td>${wl(d.w,d.l)}</td><td>${pct(d.w,t2)}</td>
      </tr>`;
    });
    h+='</tbody></table></div>';
  }
